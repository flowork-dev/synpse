...
from flask import Blueprint, jsonify, g # <-- START MODIFIED CODE (FIX)
...
def get_presets_list(): # (FIXED) Removed current_user
...
@crypto_auth_required # (FIXED) Was token_required
def get_preset_data(preset_name): # (FIXED) Removed current_user
    current_user = g.user # (FIXED) Get user from context
    try:
...
@crypto_auth_required # (FIXED) Was token_required
def save_preset_data(): # (FIXED) Removed current_user
    current_user = g.user # (FIXED) Get user from context
...
@crypto_auth_required # (FIXED) Was token_required
def delete_preset_data(preset_name): # (FIXED) Removed current_user
    current_user = g.user # (FIXED) Get user from context
    try:
        preset = Preset.query.filter_by(
            user_id=current_user.id, name=preset_name
