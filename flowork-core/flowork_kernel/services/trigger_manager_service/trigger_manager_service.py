...
class TriggerManagerService(BaseService):
    ...
    def __init__(self, kernel, service_id: str):
        ...
        self.logger.debug("Service 'TriggerManager' initialized.") # English Log
    def start(self):
        """
            ...
            if listener_entry:
                ...
                if is_installed and site_packages_path and os.path.isdir(site_packages_path):
                    if site_packages_path not in sys.path:
                        ...
                        self.logger.debug(f"Added venv path to sys.path for '{trigger_id}': {site_packages_path}") # English Hardcode
                try:
                    ...
                    if is_venv_path_added:
                        try:
                            ...
                            self.logger.debug(f"Removed venv path from sys.path for '{trigger_id}'") # English Hardcode
                        except ValueError:
                            pass
        ...
        for rule_id, rule_data in rules.items():
            ...
            if trigger_id == "cron_trigger":
                ...
                if not trigger_info.get("is_installed", False):
                    self.logger.warning(f"Cannot start listener for rule '{rule_data.get('name')}': Trigger '{trigger_id}' is not installed.") # English Hardcode
                    continue
    ...
    def _worker_install_dependencies(self, trigger_id: str, on_complete: callable):
        ...
        try:
            ...
            if not os.path.exists(requirements_path):
                self.logger.info(f"No requirements.txt found for '{trigger_id}'. Marking as installed.") # English Hardcode
                ...
                return
            self.logger.info(f"Creating venv for '{trigger_id}' at {venv_path}...") # English Hardcode
            ...
            self.logger.info(f"Installing requirements for '{trigger_id}'...") # English Hardcode
            ...
            self.logger.info(f"Installation successful for '{trigger_id}'. Creating marker file.") # English Hardcode
            with open(install_marker_path, "w") as f:
                f.write("installed") # English Hardcode
            self.loaded_triggers[trigger_id]["is_installed"] = True
            on_complete(trigger_id, True, f"Dependencies installed successfully: {result.stdout}") # English Hardcode
        except Exception as e:
            self.logger.error(f"Failed to install dependencies for '{trigger_id}': {e}") # English Hardcode
            ...
            try:
                ...
                self.logger.error(f"Failed to cleanup failed install for '{trigger_id}': {cleanup_e}") # English Hardcode
    def install_component_dependencies(self, trigger_id: str, on_complete: callable):
        ...
        """
        self.logger.info(f"Queuing dependency installation for trigger: {trigger_id}") # English Hardcode
    ...
    def _worker_uninstall_dependencies(self, trigger_id: str, on_complete: callable):
        ...
        try:
            ...
            if os.path.exists(install_marker_path):
                os.remove(install_marker_path)
                self.logger.info(f"Removed install marker for '{trigger_id}'.") # English Hardcode
            if os.path.isdir(venv_path):
                shutil.rmtree(venv_path)
                self.logger.info(f"Removed venv directory for '{trigger_id}'.") # English Hardcode
            ...
            self.logger.error(f"Failed to uninstall dependencies for '{trigger_id}': {e}") # English Hardcode
            self.logger.debug(traceback.format_exc())
    ...
    def uninstall_component_dependencies(self, trigger_id: str, on_complete: callable):
        ...
        """
        self.logger.info(f"Queuing dependency uninstallation for trigger: {trigger_id}") # English Hardcode
