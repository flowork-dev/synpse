...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\kernel_logic.py total lines 631 
########################################################################
    ...
    def __init__(self, project_root_path: str):
        ...
        if external_data_path and os.path.isdir(external_data_path):
            ...
            if external_data_path:
                print(f"[KERNEL WARNING] FLOWORK_DATA_PATH set to '{external_data_path}' but it's not a valid directory. Falling back to internal paths.") # English Hardcode
            else:
                print("[KERNEL INFO] FLOWORK_DATA_PATH not set. Using internal paths.") # English Hardcode
    ...
    def _load_services_from_manifest(self):
        ...
        try:
            ...
            essential_order = [
                ...
                "state_manager_service", # <-- FIX: Was "state_manager"
            ...
            if "state_manager_service" in self.services:
                ...
                self.write_to_log(
                    "Alias 'state_manager' created for 'state_manager_service'.", # English Hardcode
                    "SUCCESS",
                )
    ...
    def _load_service(self, service_config: Dict[str, str]):
        ...
        try:
            ...
            critical_services = [
                ...
                "state_manager_service", "variable_manager", "preset_manager_service", # <-- FIX 1: Was "state_manager"
                "module_manager_service", "startup_service",
                "gateway_connector_service", "scanner_manager_service", "api_server_service", # <-- FIX 2: Added missing critical services
                "websocket_server_service" # <-- FIX 3: Added WebSocket service (Roadmap 5)
            ]
